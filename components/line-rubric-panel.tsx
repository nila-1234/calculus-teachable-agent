...
export type GradingDisputeState = {
  speaker: "ai-student" | "professor";
  status: "continue" | "resolved";
  recommendedMark: "pass" | "fail";
...
// Keyed by answerId -> criterionId -> dispute
export type GradingDisputesState = Record<string, Record<string, GradingDisputeState>>;
...
type LineRubricPanelProps = {
  ...
  disputes?: GradingDisputesState;
  onSendDisputeMessage?: (answerId: string, criterionId: string, text: string) => void;
  onEndDispute?: (answerId: string, criterionId: string) => void;
...
export default function LineRubricPanel({
  ...
  const currentDisputes = disputes?.[currentAnswer.id] ?? {};
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_280px]">
        <div className="rounded-xl border border-stone-200 bg-white p-6 shadow-sm">
          ...
          <p className="mb-4 text-sm text-stone-500">
            ...
            <span className="font-semibold text-stone-600">AI Pass</span> if their step satisfies it,{" "}
            <span className="font-semibold text-stone-600">AI Fail</span> if it does not.
          </p>
                        ...
                        {stepPlacements.map((criterion) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              criterionFeedback &&
                              !criterionFeedback.statusCorrect &&
                              currentDisputes[criterion.id] ? (
                                <div className="ml-5 mt-1">
                                  <GradeChat
                                    compact
                                    studentLabel={
                                      currentDisputes[criterion.id].speaker === "professor"
                                        ? "Professor"
                                        : currentAnswer.label
                                    }
                                    messages={currentDisputes[criterion.id].messages}
                                    pending={currentDisputes[criterion.id].pending}
                                    status={currentDisputes[criterion.id].status}
                                    onSend={(text) =>
                                      onSendDisputeMessage?.(
                                        currentAnswer.id,
                                        criterion.id,
                                    ...
                                    onEnd={() =>
                                      onEndDispute?.(currentAnswer.id, criterion.id)
                                    }
                                  />
                                </div>
                              ) : null}
