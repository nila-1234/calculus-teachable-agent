...
async function main() {
  ...
  if (args.includes("--env") && envFilter) query.env = envFilter;
  if (args.includes("--subject") && subjectFilter) query.subject_id = subjectFilter;
  ...
  try {
    ...
    if (!docs.length) {
      console.log(`No log documents matched in ${dbName}.logs`);
      return;
    }
    ...
    const eventRows = docs.map((doc) => ({
      ...
      received_at: doc.received_at?.toISOString?.() ?? "",
      env: doc.env ?? "",
      event: doc.event ?? "",
    ...
    console.log(`Exported ${docs.length} events from ${dbName}.logs`);
